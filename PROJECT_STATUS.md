...
- **⚠ GitHub Packages token expired → blocks 2d deploy.** `npm install @caistech/report-generator` returns 401 (both `NODE_AUTH_TOKEN` and the hardcoded `~/.npmrc` token are stale — the `caistech_republish_hazard` pattern). Consequence: `package.json` declares `@caistech/report-generator@^0.1.0` but `package-lock.json` does NOT resolve it, so `npm ci`/Vercel build will fail. **Fix:** refresh the GitHub PAT with `read:packages`, set `NODE_AUTH_TOKEN` (and Vercel's env token), then `npm install` to resolve + write the lock. The 2d code is complete + build-verified locally against a staged copy of the package's built `dist/` (in `node_modules`, not committed).
