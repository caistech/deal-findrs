...
- **Phase 3 — Checklist 2/3 into the deal-model (grounded plan in `docs/estate-constraints-yield-plan.md`).** Decision: deal-model is the lot-based finance home; wire-not-rebuild; H&L is a layerable per-lot construction component.
  - [x] **3a — Bridge DONE.** Estate derived `authoritativeLots` now pre-fills the deal-model lot count (governs over `num_lots`), with a source trace + un-derivable→referral prompt. Buildup emits the `cost` gap.
  - [x] **3b — QS cost pack DONE.** Lot-level QS cost buildup (`src/lib/estate-cost/`) — land + civil/infra + soft + statutory + contingency per lot (benchmark rates × region factor via devfinance's engine), optional H&L home line. Unlocks the **QS review pack** (renders to a real PDF); feeds the deal-model per-lot inputs. Land-subdivision only for now (H&L layers when the deal-model carries a capture rate).
  - [x] **3c — GRV & absorption DONE (core).** New `src/lib/estate-valuation/`: GRV/lot (operator/study figure the valuer certifies) + a **two-phase absorption curve** (evidence-gated pre-sold burst → benchmark tail, monthly take-up vector; degrades to benchmark-only) + an independent **Domain AVM cross-check** (key-optional, confidence-gated natively by Domain's `priceConfidence`, degrades cleanly). Unlocks the **valuer review pack** (renders to a real PDF). Reality-adjusted from "real comps": Domain is estimate-only (no sold-comps table), so the AVM corroborates the site value, GRV stays operator-certified. `next build` clean; all review-pack + valuation tests green (24).
    - [x] **3c-D DONE.** `generateCashFlow` now takes an optional `salesProfile` (per-month revenue fractions) that overrides the flat even spread; `absorptionToSalesProfile` converts the absorption vector; `generateFeasibilityStudy` passes it through; the feasibility route **auto-derives it from the opportunity's pre-sales evidence** (only when pre-sales > 0 → unchanged behaviour otherwise). A front-loaded absorption curve now shortens the holding period + cuts finance cost. Tested (front-load < even interest); `next build` clean.
