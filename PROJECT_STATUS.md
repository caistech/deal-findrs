...
- **Phase 3 — Checklist 2/3 into the deal-model (grounded plan in `docs/estate-constraints-yield-plan.md`).** Decision: deal-model is the lot-based finance home; wire-not-rebuild; H&L is a layerable per-lot construction component.
  ...
  - [ ] **3c — GRV & absorption (Checklist 3).** GRV/lot from **real Domain comps** (replace AI-synthetic; the backend shipped 2026-05-27, wiring pending) + a demand-backed absorption curve from `waitlist_register` platform-demand (replace the scalar heuristic) → unlock the **valuer review pack** → feeds deal-model `marketPricePerLot`.
